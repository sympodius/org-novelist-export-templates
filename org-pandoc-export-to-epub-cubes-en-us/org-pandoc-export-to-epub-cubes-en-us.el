    ...
    (if valid-replacement-graphic-p
      ...
      (let (cover-graphic-arg
        ...
        (if (> (length (opeteceu--get-file-property-value "TITLE"  org-input-file)) 22)
            ...
            (progn
              (setq title-words (split-string (opeteceu--get-file-property-value "TITLE" org-input-file) " "))  ; What text we're actually splitting up, stored as words
          ...
          (setq title-graphic-arg (concat "-draw \"line 100,1120 1500,1120\" -draw \"line 100,1440 1500,1440\" -draw \"text 0," (number-to-string curr-title-pos) " \'" (opeteceu--get-file-property-value "TITLE"  org-input-file) "\'\" ")))
        ;; If subtitle longer than x, split into lines. Else, return subtitle.
        (if (> (length (opeteceu--get-file-property-value "SUBTITLE" org-input-file)) 56)
            ;; Subtitle too long, split it up.
            (progn
              (setq subtitle-words (split-string (opeteceu--get-file-property-value "SUBTITLE" org-input-file) " "))  ; What text we're actually splitting up, stored as words
          ...
          (setq subtitle-graphic-arg (concat "-draw \"text 0," (number-to-string curr-subtitle-pos) " \'" (opeteceu--get-file-property-value "SUBTITLE" org-input-file) "\'\" ")))
        ;; If author longer than x, split into lines. Else, return author.
        (if (> (length (opeteceu--get-file-property-value "AUTHOR" org-input-file)) 22)
            ;; Author too long, split it up.
            (progn
              (setq author-words (split-string (opeteceu--get-file-property-value "AUTHOR" org-input-file) " "))  ; What text we're actually splitting up, stored as words
              (setq curr-word nil)
              (setq curr-line "")
          ...
          (setq author-graphic-arg (concat "-draw \"text 0," (number-to-string curr-author-pos) " \'" (opeteceu--get-file-property-value "AUTHOR" org-input-file) "\'\" ")))
        ;; If Imagemagick is available, generate cover.
        (when (and (executable-find "magick") (file-readable-p opeteceu--title-page-graphic))
          (shell-command (concat "magick -size 1600x2560 xc:white -fill black -stroke black -font \"Josefin-Sans\" -pointsize 126 -gravity center " title-graphic-arg " -font \"Josefin-Sans-Italic\" -pointsize 54 " subtitle-graphic-arg " -font \"Alegreya-SC\" -pointsize 72 " author-graphic-arg cover-graphic-arg " \"" cover-output-file "\""))
          (setq opeteceu--title-page cover-output-file))))))
